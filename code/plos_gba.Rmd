...
analysis_sf_clean <- analysis_sf %>%
  filter(!is.na(walk_rate), !is.na(prop_women))

nrow(analysis_sf_clean)
```

Converting to sp object

```{r}
analysis_sp <- as(analysis_sf_clean, "Spatial")
```
...
```{r}
analysis_sf_clean$coef_women <- gwr_model$SDF$prop_women

summary(analysis_sf_clean$coef_women)
```

Mapping coefficients

```{r}
coef_map <- ggplot(analysis_sf) +
  geom_sf(aes(fill = coef_women), color = NA) +
  scale_fill_viridis_c(
    direction = 1,
    name = "Local coefficient\n(prop. women & walk rate)",
    na.value = "white"
  ) +
  labs(
